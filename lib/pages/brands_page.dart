...
class _AddEditBrandDialogState extends State<AddEditBrandDialog> {
  ...
  Future<void> loadCategories() async {
    try {
      ...
      categories = raw.map<Map<String, dynamic>>((c) => {
  ...
  Future<void> loadSubcategories(int categoryId) async {
    try {
      final raw = await ApiService.getSubcategories(categoryId);

      subcategories = raw.map<Map<String, dynamic>>((s) => {
            'SubcategoryID': s['SubcategoryID'] ?? s['subcategory_id'],
            'Name': s['Name'] ?? s['name'] ?? 'Unnamed',
          }).toList();

      if (!subcategories
          .any((s) => s['SubcategoryID'] == selectedSubcategoryId)) {
        selectedSubcategoryId = null;
      }
            ...
            children: [
              ...
              DropdownButtonFormField<int>(
                ...
                onChanged: (val) async {
                  ...
                  if (val != null) await loadSubcategories(val);
                },
